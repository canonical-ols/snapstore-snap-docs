...
```bash
sudo enterprise-storeconfig proxy.oci-registry.domain=<registry-domain>
```
If required, set the credentials for registry access.
```bash
sudo enterprise-storeconfig proxy.oci-registry.username=some-username proxy.oci-registry.password=some-password
```
...
```bash
sudo enterprise-storepush-charms /var/snap/snap-store-proxy/common/charms-to-push/charms-export-20240429T090849.tar.gz
```

Example of importing a `cos-lite` bundle:

```bash
sudo enterprise-storepush-charm-bundle /var/snap/snap-store-proxy/common/charms-to-push/cos-lite-20240401T172030.tar.gz
```
...
```bash
sudo enterprise-storeimport-certificate --selfsigned
```

After it's created, you can retrieve the public key from the configuration:
```bash
enterprise-storeconfig proxy.tls.cert | cat > tls-cert.crt
```
